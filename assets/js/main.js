...
$('.form-ajax').submit(function(e) {
	...
	$.ajax({
		...
		success: function(json) {	            
			...
			$('.form-ajax label[for="name-contact"] .alert-error').fadeOut('fast', function(){
				if(json.nameMessage != '') {
					$(this).html('(' + json.nameMessage + ')').fadeIn('fast');
				}
			});
			$('.form-ajax label[for="mail-contact"] .alert-error').fadeOut('fast', function(){
				if(json.emailMessage != '') {
					$(this).html('(' + json.emailMessage + ')').fadeIn('fast');
				}
			});
			$('.form-ajax label[for="subject-contact"] .alert-error').fadeOut('fast', function(){
				if(json.subjectMessage != '') {
					$(this).html('(' + json.subjectMessage + ')').fadeIn('fast');
				}
			});
			$('.form-ajax label[for="message-contact"] .alert-error').fadeOut('fast', function(){
			...
			if(json.nameMessage == '' && json.emailMessage == '' && json.subjectMessage == '' && json.messageMessage == '') {
                $('.ajax-message').html('<h2>Thanks for contacting us! We will get back to you very soon.</h2>');
			}
